...
public class RC_ExplosiveChainAI extends RC_BaseMissile {
    ...
    private real_combat.weapons.ai.RC_ExplosiveChainAI.Stage stage = real_combat.weapons.ai.RC_ExplosiveChainAI.Stage.OUT;
    ...
    public void advance(float amount) {
        ...
        if (target instanceof ShipAPI) {
            ...
            if(missile.getFlightTime()>=missile.getMaxFlightTime()*0.8f)
            {
                ...
                //当距离足够近设置状态
                if (distance<=0) {
                    if (stage != real_combat.weapons.ai.RC_ExplosiveChainAI.Stage.TO_IN) {
                        stageAngle = missileToShipAngle;
                    }
                    missileCommandNoSheild(mi, stageAngle, amount);
                    stage = real_combat.weapons.ai.RC_ExplosiveChainAI.Stage.TO_IN;
                }
                else if (distance>=0&& real_combat.weapons.ai.RC_ExplosiveChainAI.Stage.TO_IN==stage){
                    if (stage!= real_combat.weapons.ai.RC_ExplosiveChainAI.Stage.RETURN) {
                        stageAngle = missileToShipAngle;
                    }
                    missileCommandNoSheild(mi, stageAngle, amount);
                    stage = real_combat.weapons.ai.RC_ExplosiveChainAI.Stage.RETURN;
                }
                else {
                    now_circle += amount;
                    //顺势转开
