...
public class RC_TrinityForceSystemBack2 extends BaseShipSystemScript {
                        ...
                        if (m.isAlive()) {
                            ...
                            if (alive>newShips.size()){
                                ...
                                if (ship.getFleetMember().getFleetData() != null) {
                                    //ship.getFleetMember().getFleetData().addFleetMember(newFleetMember);
                                }
                                //降低部署点
                                //newFleetMember.getStats().getDynamic().getMod(Stats.DEPLOYMENT_POINTS_MOD).modifyPercent(ID, -100f);
                            ...
                            //删除模块组件
                            //newFleetMember.getVariant().removePermaMod("RC_TrinityForceModule");

                            //移除新船的飞机
                            /*
                            FighterWingAPI wing = newShip.getWing();
                            ...
                            */
                            ...
                            MutableShipStatsAPI nstats = newShip.getMutableStats();
                            //禁用甲板
                            //int bays = (int) Math.round(nstats.getNumFighterBays().getBaseValue());
                            //nstats.getNumFighterBays().modifyFlat(ID, -bays);
                            //nstats.getFighterRefitTimeMult().modifyFlat(ID,100000f);
                             ...
                             */
                            //禁用甲板
                            //int bays = (int) Math.round(mstats.getNumFighterBays().getBaseValue());
                            //mstats.getNumFighterBays().modifyFlat(ID, -bays);
                            //mstats.getFighterRefitTimeMult().modifyFlat(ID,100000f);
                        }
                    }
                    if(alive == 0)
                    {
        ...
        if (ship.getCustomData().get(ID) == null) {
            ...
            for (FighterLaunchBayAPI b : fighterLaunchBays) {
                ...
                if (b.getWing()!=null) {
                    List<ShipAPI> fighters = b.getWing().getWingMembers();
                    for (ShipAPI f:fighters) {
                        b.getLandingLocation(f).set(newLocation);
                    }
                }
            }
            */
    ...
    private class RC_TrinityForceSystemCombatPlugin implements EveryFrameCombatPlugin {
        ...
        public void advance(float amount, List<InputEventAPI> events) {
            ...
            try {
                //将飞去找newShip的飞机拉回来 废弃
                /*
                for (ShipAPI f:engine.getShips())
                {
                    if(f.isFighter())
                    {
                        if(f.getWing().getSourceShip()!=null)
                        {
                            ShipAPI newShip = f.getWing().getSourceShip();
                            if(newShip.isPhased())
                            {
                                if (newShip.getCustomData().get(ORIGINAL)!=null)
                                {
                                    ShipAPI m = (ShipAPI) newShip.getCustomData().get(ORIGINAL);
                                    if(!m.isPhased()) {
                        ...
                        if (m.getCustomData().get(ORIGINAL) != null) {
                            ...
                            for (FighterWingAPI w:wings)
                            {
                                for (ShipAPI f:w.getWingMembers()) {
                                    ...
                                    for (FighterLaunchBayAPI b : fighterLaunchBays) {
                                        if (b.getWing()!=null) {
                                            if (b.getWing().getSpec().equals(w.getSpec())) {
                                                Vector2f landingLocation = b.getLandingLocation(f);
                                                ShipAIPlugin ai = f.getShipAI();
                                                if(f.getCustomData().get(ID)==null) {
                                                    f.setShipAI(new RC_ModulesFighterAI(f, newShip, m,null, null));
                                                    f.setCustomData(ID,true);
                                                }
                                            }
                                        }
                            ...
                            */
                            if (!newShip.isAlive()) {
                                ...
                                if (nowDistance > newShip.getCollisionRadius() + ship.getCollisionRadius()) {
                                    ...
                                    //解开甲板禁用
                                    //MutableShipStatsAPI nstats = m.getMutableStats();
                                    //nstats.getNumFighterBays().unmodify(ID);
                                    //nstats.getFighterRefitTimeMult().unmodify(ID);
                                }
                            }
                            if ( outCount >= ship.getChildModulesCopy().size() - 1)
                            {
                    ...
                    if (ship.getCustomData().get(ID)!=null)
                    {
                        switch (step){
                            case 0:ship.getFluxTracker().setCurrFlux(ship.getMaxFlux()/2+ship.getCurrFlux());
                                    ...
                                    step=2;
                                /*测试战斗中模块变更
                                ShipVariantAPI variant = Global.getSettings().getVariant("hyperion_plus_variant");

                                List<String> slotIds = variant.getModuleSlots();
                                for (String s : slotIds) {
                                    ShipVariantAPI modeVariant = variant.getModuleVariant(s);
                                    variant.setSource(VariantSource.REFIT);
                                    modeVariant.setSource(VariantSource.REFIT);
                                    ShipVariantAPI newVariant = Global.getSettings().getVariant("conquest_Standard");
                                    newVariant.setSource(VariantSource.REFIT);
                                    variant.setModuleVariant(s, newVariant);
                                }
                                ...
                                ShipAPI testShip = Global.getCombatEngine().getFleetManager(FleetSide.PLAYER)
                                        .spawnFleetMember(member, new Vector2f(), 0, 0f);

                                variant = testShip.getVariant();
                                for (String s : slotIds) {
                                    ShipVariantAPI modeVariant = variant.getModuleVariant(s);
                                    variant.setSource(VariantSource.REFIT);
                                    modeVariant.setSource(VariantSource.REFIT);
                                    ShipVariantAPI newVariant = Global.getSettings().getVariant("onslaught_Standard");
                                    newVariant.setSource(VariantSource.REFIT);
                                    variant.setModuleVariant(s, newVariant);
                                }
                                 */
                                    ...
                                    if (m.getCustomData().get(ORIGINAL)!=null)
                                    {
                                        ...
                                        if (newShip.isAlive()) {
                                            ...
                                            if (newShip.getAlphaMult() < 0) {
                                                ...
                                                newShip.getLocation().set(newLocation);
                                                /*
                                                RippleDistortion ripple = new RippleDistortion(newShip.getLocation(), new Vector2f());
                                                ripple.setSize(newShip.getCollisionRadius());
                                                ripple.setIntensity(100f);
                                                ripple.fadeOutSize(0.4f);
                                                ...
                                                */
                                                ...
                                                for (FighterWingAPI w : wings) {
                                                    for (ShipAPI f : w.getWingMembers()) {
                                                        ShipAIPlugin ai = f.getShipAI();
                                                        /*
                                                        if (Math.abs(f.getLocation().getX())>engine.getMapWidth()&&Math.abs(f.getLocation().getY())>engine.getMapHeight())
                                                        {
                                                            f.splitShip();
                                                        }
                                                        else
                            ...
                            case 4:
                                    ...
                                    if (m.getCustomData().get(ORIGINAL)!=null)
                                    {
                                        ...
                                        if (newShip.isAlive()) {
                                            ...
                                            if (nowDistance < MIN_DISTANCE) {
                                                ...
                                                if (ship.getFleetMember().getFleetData()!=null) {
                                                    //ship.getFleetMember().getFleetData().removeFleetMember(newShip.getFleetMember());
                                                }
                                                //newShip.splitShip();
                                                newShip.setAlphaMult(0);
                                                ...
                                                /*
                                                m.setShipSystemDisabled(true);
                                                float arc = (Float) m.getCustomData().get(ARC);
                                                //m.getShield().setArc(arc);
                                                MutableShipStatsAPI mstats = m.getMutableStats();
                                                ...
                                                */
                                                //解开甲板禁用
                                                //mstats.getNumFighterBays().unmodify(ID);
                                                //mstats.getFighterRefitTimeMult().unmodify(ID);
                                            } else {
                                ...
                                if (allReady == ship.getChildModulesCopy().size())
                                {
                                    ...
                                    CombatFleetManagerAPI manager = engine.getFleetManager(ship.getOwner());
                                    FleetMemberAPI newFleetMember = Global.getFactory().createFleetMember
                                            (FleetMemberType.SHIP, ship.getVariant());
                                    newFleetMember.setOwner(ship.getOwner());
                                    newFleetMember.getCrewComposition().addCrew(newFleetMember.getNeededCrew());
                                    List<String> slotIds = ship.getVariant().getModuleSlots();
                                    for (String s:slotIds)
                                    {
                                        newFleetMember.getVariant().setModuleVariant(s,Global.getSettings().getVariant("onslaught_Standard"));
                                    }
                                    float height = engine.getMapHeight();
                                    Vector2f newLocation = new Vector2f(ship.getCollisionRadius() * 4, ship.getOwner() == 0 ? height / -2f - ship.getCollisionRadius() * 4 : height / 2f + ship.getCollisionRadius() * 4);
